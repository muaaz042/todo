...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotes = async () => {
      try {
        const res = await axios.get(`${BACKEND_URL}/note/getNotes`, config);
  ...
  const colors = ["bg-green-300/40", "bg-purple-300/40", "bg-blue-300/40"];
  ...
  return (
    <div>
      <div
        className={`${getRandomColor()} w-full h-52 px-4 flex justify-center items-center gap-8 relative mb-10 shadow-lg`}
      >
        <p className="text-4xl ">
          click on <strong>+</strong> button to add note
        </p>
