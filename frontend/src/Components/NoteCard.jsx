...
const NoteCard = ({ index, bgColor, note, onDelete }) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const res = await axios.delete(
        `${BACKEND_URL}/note/deleteNote/${note._id}`,
        {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-between p-5 gap-4 lg:w-2/4 md:w-3/4 w-full shadow-xl rounded-3xl ${bgColor}`}
    >
